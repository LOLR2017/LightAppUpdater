...
open class TLH_HitView: UIView {
    ...
    func createContentView() -> UIView {
        if backView == nil {
            backView = UIView.init(frame: CGRect.init(x: self.frame.width/2-140, y: self.frame.height/2-150, width: 280, height: 300))
            backView?.backgroundColor = UIColor.white
            backView?.layer.cornerRadius = 4
            backView?.layer.masksToBounds = true
            backView?.isUserInteractionEnabled = true
            
            titleLabel = UILabel.init(frame: CGRect.init(x: 0, y: 15, width: (backView?.frame.width)!, height: 20))
            titleLabel?.text = "新版本升级提醒"
            titleLabel?.textAlignment = .center
            titleLabel?.textColor = UIColor.black
            titleLabel?.font = UIFont.boldSystemFont(ofSize: 20)
            backView?.addSubview(titleLabel!)
            
            contentLabel = UILabel.init(frame: CGRect.init(x: 30, y: (titleLabel?.frame.maxY)!+25, width: (backView?.frame.width)!-60, height: 20))
            ...
            finishButton?.setTitleColor(UIColor.init(red: 47/255.0, green: 134/255.0, blue: 253/255.0, alpha: 1.0), for: .normal)
            ...
            firstLineView?.backgroundColor = UIColor.lightGray
            ...
            secondLineView?.backgroundColor = UIColor.lightGray
